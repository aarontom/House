...
router.get('/:id', (req, res) => {
  try {
    ...
    const positionsWithValue = positions.map(p => {
      ...
      if (p.market_status === 'resolved' && p.outcome) {
        const won = p.outcome === p.position_type;
        // Winning positions: $1 per share payout
        // Losing positions: $0 payout
        const payout = won ? p.shares * 1.0 : 0;
        const pnl = payout - costBasis;
        
        console.log(`[Portfolio] Position ${p.id}: market=${p.market_title}, status=${p.market_status}, outcome=${p.outcome}, position=${p.position_type}, won=${won}, shares=${p.shares}, costBasis=$${costBasis.toFixed(2)}, payout=$${payout.toFixed(2)}, pnl=$${pnl.toFixed(2)}`);
        
        // Resolved positions don't add to total value (already paid out to balance)
        return {
          ...p,
          current_price: won ? 1.0 : 0.0, // Resolved price: $1 if won, $0 if lost
      ...
      // For open markets, use current market price
      const marketState = {
  ...
  try {
    ...
    const positionsWithValue = positions.map(p => {
      ...
      if (p.market_status === 'resolved' && p.outcome) {
        ...
        console.log(`[Positions] Position ${p.id}: market=${p.market_title}, status=${p.market_status}, outcome=${p.outcome}, position=${p.position_type}, won=${won}, shares=${p.shares}, costBasis=$${costBasis.toFixed(2)}, payout=$${payout.toFixed(2)}, pnl=$${pnl.toFixed(2)}`);
